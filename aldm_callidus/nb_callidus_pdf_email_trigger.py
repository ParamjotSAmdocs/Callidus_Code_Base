...
if run_mid_monthly == 'true':
    ...
    remove_files_by_extension(f"{mft_out}/aldm/outbound_1/POC/INVOICE", ".pdf")
    ...
    try:
        merge_and_archive_pdfs_mft("POC/INVOICE", "POC/INVOICE/FINAL", f"{INVOICE_TYPE}_INVOICE_PDF_{period}_{Sequence}_APPROVED_{invoice_run_dt}.pdf",INVOICE_TYPE)
...
if run_monthly == 'true':
    ...
    # Merge PDFs, archive, and move files to Azure File Share
    try:
        merge_and_archive_pdfs_mft("POC/INVOICE", "POC/INVOICE/FINAL", f"{INVOICE_TYPE}_INVOICE_PDF_{period_file}_{Sequence}_APPROVED_{invoice_run_dt}.pdf", INVOICE_TYPE)
        archive_files_at_azure_share(fs_conn_str, fs_name, "callidus/INVOICE", INVOICE_TYPE)
        move_files_to_azure_share(fs_conn_str, fs_name, f"{mft_out}/aldm/outbound_1/POC/INVOICE/INDIVIDUAL", "callidus/INVOICE", INVOICE_TYPE)
        move_files_to_azure_share(fs_conn_str, fs_name, f"{mft_out}/aldm/outbound_1/POC/INVOICE/FINAL", "callidus/INVOICE", INVOICE_TYPE)
    except Exception as e:
