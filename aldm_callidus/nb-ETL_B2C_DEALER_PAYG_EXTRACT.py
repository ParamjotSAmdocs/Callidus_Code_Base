...
DELETE FROM {sfDatabase}.PRS_CEG_EXT.B2C_DEALER_PAYG_EXTRACT
...
INSERT INTO {sfDatabase}.PRS_CEG_EXT.B2C_DEALER_PAYG_EXTRACT (
    PERIOD_NAME,
    PARTNER_CODE,
    PARTNER_NAME,
    SERVICE_TYPE,
...
# MAGIC import os
# MAGIC # from azure.storage.fileshare import ShareFileClient
# MAGIC # from azure.storage.fileshare import ShareDirectoryClient
# MAGIC
# MAGIC # Set extract location and filename pattern
# MAGIC lv_extract_location = '/Volumes/sit_aldm/aldm_oper/mft-out/aldm/outbound_1/B2C_DEALER_DETAILED_STATEMENT_PAYG'
# MAGIC lv_ext_filename = 'DEALER_DETAILED_STATEMENT_PAYG_MMYYYY_<SEQ>.csv'
# MAGIC
# MAGIC try:
# MAGIC     # Check if DataFrame has rows to export
# MAGIC     if df_data.shape[0] > 0:
# MAGIC         print("DataFrame has rows. Proceeding with file generation.")
# MAGIC         lv_ext_filename_final, ext_filename_pttrn, month_str, seq_str = file_seq_generator(lv_ext_filename,month_str)
# MAGIC         print("Final Extract Name: ", lv_ext_filename_final)
# MAGIC         print("Extract Filename Pattern: ", ext_filename_pttrn)
# MAGIC         print("Extract Month :", month_str)
# MAGIC         print("Extract Seq :", seq_str)
# MAGIC
# MAGIC         # Write DataFrame to CSV
# MAGIC         df_data.to_csv(os.path.join(lv_extract_location, lv_ext_filename_final), index=False, header=True)
# MAGIC         print(f"CSV file '{lv_ext_filename_final}' generated successfully at '{lv_extract_location}'.")
# MAGIC
# MAGIC         # Archive old files in Azure File Share
# MAGIC         print("Archiving old files in Azure File Share...")
# MAGIC         archive_files_at_azure_share(fs_conn_str, fs_name, "callidus/B2C_DEALER_DETAILED_STATEMENT_PAYG", "DEALER_DETAILED_STATEMENT_PAYG")
# MAGIC         print("Old files archived successfully.")
# MAGIC
# MAGIC         # Move new file to Azure File Share
# MAGIC         print("Uploading new file to Azure File Share...")
# MAGIC         move_files_to_azure_share(fs_conn_str, fs_name, f"{mft_out}/aldm/outbound_1/B2C_DEALER_DETAILED_STATEMENT_PAYG", "callidus/B2C_DEALER_DETAILED_STATEMENT_PAYG", "DEALER_DETAILED_STATEMENT_PAYG")
# MAGIC         print("New file uploaded successfully.")
# MAGIC
# MAGIC     else:
# MAGIC         print("DataFrame is empty. Skipping CSV file generation and Azure upload.")
# MAGIC except Exception as e:
# MAGIC     print(f"Error encountered: {e}")
# MAGIC     raise  # Fail notebook if any error occurs
...
# lv_extract_location = '/Volumes/dev_aldm/aldm_oper/mft-out/aldm/outbound/B2C_DELAER_DETAILED_STATEMENT_PAYG'
# lv_ext_filename = 'DEALER_DETAILED_STATEMENT_PAYG_MMYY_<SEQ>.csv'
...
#         select_seq_sql = f"""
#         SELECT SEQ_NUM FROM EXTRACT_SEQ_TRACK
...
#             seq_num = 1
#             insert_seq_sql = f"""
#             INSERT INTO EXTRACT_SEQ_TRACK (EXTRACT_NAME, EXTRACT_MONTH, SEQ_NUM)
...
#         connection_string = "DefaultEndpointsProtocol=https;AccountName=dlsaldmdevweu001;AccountKey=<SET_FROM_KEYVAULT>;EndpointSuffix=core.windows.net"  ########HAVE TO MOVE TO  KEYVALULT INSTEAD OF HARDCODING 
#         share_name = 'aldm-callidus'      #### TBD SHARE_NAME
#         dir_path = 'callidus/PAYG/'        #### WILL BE DIFF FOR DIFF EXTRACTS
