...
if if_email_trigger == "true":        # Job 1 email_trigger
    ...
    for f in files:
        ...
        period      = f["period"]
        Sequence    = f["sequence"]
    period_name_value = period[:3] + period[5:7] + period[-1]
    print(period_name_value)
else:                             # Job 2 BAU
    period_name_value = dbutils.jobs.taskValues.get(taskKey="Callidus_Trigger_Check", key="PERIOD_NAME", debugValue="DEBUG")
    print(period_name_value) 
...
DELETE FROM {sfDatabase}.PRS_CEG_EXT.CALLIDUS_INVOICE_PDF_MIDMONTHLY
--WHERE REGEXP_LIKE(PERIOD_NAME, '^' || TO_VARCHAR(DATEADD(MONTH, -1, CURRENT_DATE), 'MonYY')  || '[A-E]$', 'i')
...
INSERT INTO {sfDatabase}.PRS_CEG_EXT.CALLIDUS_INVOICE_PDF_MIDMONTHLY
SELECT
...
FROM (
	...
	--WHERE REGEXP_LIKE(PERIOD, '^' || TO_VARCHAR(DATEADD(MONTH, -1, CURRENT_DATE), 'MonYY')  || '[A-E]$', 'i')
